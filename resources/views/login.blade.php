            ...
            <div class="flex flex-col w-full max-w-full px-3 mx-auto md:flex-0 shrink-0 md:w-6/12 lg:w-5/12 xl:w-4/12">
              <div class="relative flex flex-col min-w-0 mt-32 break-words bg-transparent border-0 shadow-none rounded-2xl bg-clip-border">
                <div class="p-6 pb-0 mb-0 bg-transparent border-b-0 rounded-t-2xl">
                  <h3 class="relative z-10 font-bold text-transparent bg-gradient-to-tl from-blue-600 to-cyan-400 bg-clip-text">Selamat Datang di Marem!</h3>
                  <p class="mb-0">Masukkan email dan password untuk sign in.</p>
                </div>
                {{-- ALERT STATUS --}}
                @if (session('loginError'))
                    <div id="loginErrorAlert" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
                      <strong class="font-bold">Error!</strong>
                      <span class="block sm:inline">{{ session('loginError') }}</span>
                    </div>

                      <!-- Tambahkan skrip JavaScript untuk menyembunyikan alert setelah beberapa detik -->
                      <script>
                        document.addEventListener('DOMContentLoaded', function() {
                          ...
                          }, 2000); // Sesuaikan dengan jumlah milidetik yang diinginkan (dalam contoh ini, 5000 milidetik atau 5 detik)
                        });
                      </script>
                @endif
                {{-- END ALERT STATUS --}}
                <div class="flex-auto p-6">
                  <form action="/login" method="POST">
                    ...
                    <div class="mb-4">
                      <input type="email" id="name" name="email" class="focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow" placeholder="Email" aria-label="Email" aria-describedby="email-addon" />
                    </div>
